...
function inspect(root, targetFile) {
  var targetFileFullPath = path.resolve(root, targetFile);

  var targetFileRoot = path.dirname(targetFileFullPath);
  var targetFileName = path.basename(targetFileFullPath);

  // In case if we use govendor, root of the project should be one level up
  // to vendor folder.
  if (endsWith(targetFileFullPath, 'vendor/vendor.json')) {
    targetFileRoot = path.resolve(targetFileRoot, '..');
    targetFileName = path.join('vendor', targetFileName);
  }

  return Promise.all([
    getMetaData(targetFileRoot, targetFileName),
    getDependencies(targetFileRoot, targetFileName),
  ])
