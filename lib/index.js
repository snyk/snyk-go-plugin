...
function getProjectRootFromTargetFile(targetFile) {
  ...
  if (
    parts[parts.length - 1] == 'Godeps.json' &&
    parts[parts.length - 2] == 'Godeps') {
    return path.dirname(path.dirname(resolved));
  }
...
function parseConfig(root, targetFile) {
  ...
  switch (pkgManager) {
    ...
    case 'godep': {
      return parseGodepLock(root, targetFile);
    }
    default: {
      throw new Error('Unsupported file:', targetFile);
    }
...
function parseGodepLock(root, targetFile) {
  var lock = fs.readFileSync(path.join(root, targetFile));

  var deps = {};
  var lockJson = JSON.parse(lock);

  lockJson.Deps && lockJson.Deps.forEach(function (pkg) {
    var version;
    // Godep saves the output of `git describe` in the "Comment" field.
    // `git describe` may be a "valid" semver prerelease string,
    // but it doesn't behave correctly with comparison, see:
    //   https://github.com/mojombo/semver/issues/106
    // so we ignore all prerelease semver versions
    var comment = pkg.Comment;
    if (semver.valid(comment) && !semver.prerelease(comment)) {
      version = comment;
    } else {
      version = '#' + pkg.Rev;
    }
